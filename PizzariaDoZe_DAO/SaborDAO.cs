...
namespace PizzariaDoZe_DAO
{
    ...
    public class SaborDAO
    {
        ...
        public int Inserir(Sabor sabor)
        {
            ...
            try
            {
                ...
                foreach (Ingrediente auxIngrediente in sabor.SaborIngredientes)
                {
                    ...
                    comando.CommandText = @"INSERT INTO itens_sabores(sabor_id, ingrediente_id) VALUES (" + Convert.ToInt32(IdSaborGerado) + "," + auxIngrediente.Id_ingrediente + ")";
        ...
        public DataTable Buscar(Sabor sabor)
        {
            ...
            "(SELECT GROUP_CONCAT(i.nome SEPARATOR ', ') FROM itens_sabores AS iss, tb_ingrediente i WHERE iss.sabor_id = s.id_sabor AND i.id_ingrediente = iss.ingrediente_id ) AS Ingredientes " + "FROM tb_sabores AS s " + auxSqlFiltro +
